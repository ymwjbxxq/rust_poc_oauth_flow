...
impl User {
    pub fn to_dynamodb(&self) -> Result<HashMap<String, AttributeValue>, ApplicationError> {
        ...
        retval.insert(
            "client_id".to_owned(),
            AttributeValue::S(self.client_id.as_ref().unwrap().clone()),
        );
        retval.insert(
            "email".to_owned(),
            AttributeValue::S(self.email.as_ref().unwrap().clone()),
        );
        retval.insert(
            "password".to_owned(),
            AttributeValue::S(self.password.as_ref().unwrap().clone()),
        );
        retval.insert(
            "family_name".to_owned(),
            AttributeValue::S(self.family_name.as_ref().unwrap().clone()),
        );
        retval.insert(
            "given_name".to_owned(),
            AttributeValue::S(self.given_name.as_ref().unwrap().clone()),
        );
        retval.insert(
            "is_consent".to_owned(),
            AttributeValue::Bool(self.is_consent.unwrap_or_default()),
        );
        retval.insert(
            "is_optin".to_owned(),
            AttributeValue::Bool(self.is_optin.unwrap_or_default()),
        );
