...
impl CookieExt for Cookie<'_> {
  ...
  fn to_cookie_string(cookie_name: String, cookie_hash_map: HashMap<String, String>) -> String {
    let cookie = Cookie::build(cookie_name, serde_json::to_string(&cookie_hash_map).unwrap())
      .path("/")
      .secure(true)
      .http_only(true)
      .finish();
...
impl CookieHelper {
  pub fn from_http_header(headers: &HeaderMap<HeaderValue>) -> Result<HashMap<String, String>, ApplicationError>  {
