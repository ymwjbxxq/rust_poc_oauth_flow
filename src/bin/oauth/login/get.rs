...
use oauth_flow::utils::injections::oauth::login::get_di::{GetAppClient, GetAppInitialisation};
use serde_json::json;
...
pub async fn execute(
    ...
    if let Some(login_page) = login_page {
        let url = format!(
            "{}://{}{}",
        ...
        let to_html = CriptoHelper::from_base64(login_page)?.replace("#url#", &url);
